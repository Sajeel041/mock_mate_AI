...
// Aggregate stats for the dashboard top strip. Demo-friendly: always
// returns at least the demo feedback so a fresh user sees something.
export async function getUserStats(userId: string): Promise<UserStats> {
  if (!userId) {
    return { interviewsTaken: 0, averageScore: 0, bestScore: 0, practiceMinutes: 0 };
  }
  ...
  try {
    const snap = await db
      ...
      .get();
    scores = snap.docs
      .map((d) => (d.data() as Feedback).totalScore)
      .filter((n) => typeof n === "number");
  } catch {
    scores = [];
  }

  // Always include demo feedback so the dashboard has signal on first run.
  if (scores.length === 0) {
    scores = [demoFeedback.totalScore];
  }

  const interviewsTaken = scores.length;
  const sum = scores.reduce((a, b) => a + b, 0);
  const averageScore = Math.round(sum / interviewsTaken);
  const bestScore = Math.max(...scores);
  // Rough estimate: ~8 min per session. Replace if you start tracking real durations.
  const practiceMinutes = interviewsTaken * 8;

  return { interviewsTaken, averageScore, bestScore, practiceMinutes };
}
